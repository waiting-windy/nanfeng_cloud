package com.nanfeng.nanfeng_cloud.controller;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.nanfeng.nanfeng_cloud.entity.FileFolder;
import com.nanfeng.nanfeng_cloud.entity.FileStoreStatistics;
import com.nanfeng.nanfeng_cloud.entity.MyFile;
import com.nanfeng.nanfeng_cloud.utils.LogUtils;
import org.slf4j.Logger;
import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.*;

import java.util.*;

/**
 * @Description: 系统页面跳转控制器
 **/
@Controller
public class SystemController extends BaseController {

    Logger logger = LogUtils.getInstance(SystemController.class);

    /**
     * @Description 前往我的网盘
     * @Author  nanfeng
     * @Date  9:34 2020/3/28
     * @Param [fId, fName, error, map]
     * @Return java.lang.String
     */
    @GetMapping("/files")
    public String toFileStorePage(Integer fId, String fName, Map<String, Object> map) {

        //包含的子文件夹
        List<FileFolder> folders = null;
        //包含的所有文件
        List<MyFile> files = null;
        //当前文件夹
        FileFolder nowFolder = null;
        //当前文件夹的相对路径
        List<FileFolder> location = new ArrayList<>();
        if (fId == null || fId <= 0) {
            //代表当前为根目录
            fId = 0;
            folders = fileFolderService.getRootFoldersByFileStoreId(loginUser.getFileStoreId());
            files = myFileService.getRootFilesByFileStoreId(loginUser.getFileStoreId());
            nowFolder = FileFolder.builder().fileFolderId(fId).build();
            location.add(nowFolder);
        } else {
            //当前为具体目录,访问的文件夹不是当前登录用户所创建的文件夹
            FileFolder folder = fileFolderService.getFileFolderById(fId);
            if (folder.getFileStoreId() - loginUser.getFileStoreId() != 0){
                return "redirect:/files";
            }
            //当前为具体目录，访问的文件夹是当前登录用户所创建的文件夹
            folders = fileFolderService.getFileFolderByParentFolderId(fId);
            files = myFileService.getFilesByParentFolderId(fId);
            nowFolder = fileFolderService.getFileFolderById(fId);
            //遍历查询当前目录
            FileFolder temp = nowFolder;
            //由此文件开始不断向父目录遍历，直到根目录，此时得到文件绝对路径，是反向的
            while (temp.getParentFolderId() != 0) {
                temp = fileFolderService.getFileFolderById(temp.getParentFolderId());
                location.add(temp);
            }
        }
        Collections.reverse(location);

        //获得统计信息
        FileStoreStatistics statistics = myFileService.getCountStatistics(loginUser.getFileStoreId());
        map.put("statistics", statistics);
        map.put("folders", folders);
        map.put("files", files);
        map.put("nowFolder", nowFolder);
        map.put("location", location);
        logger.info("网盘页面域中的数据:" + map);
        return "u-admin/files";
    }

    /**
     * @Description 前往文件上传页面
     * @Author xw
     * @Date 15:16 2020/2/26
     * @Param [fId, fName, map]
     * @return java.lang.String
     **/
    @GetMapping("/upload")
    public String toUploadPage(Integer fId, String fName, Map<String, Object> map) {
        //包含的子文件夹
        List<FileFolder> folders = null;
        //当前文件夹信息
        FileFolder nowFolder = null;
        //当前文件夹的相对路径
        List<FileFolder> location = new ArrayList<>();
        if (fId == null || fId <= 0) {
            //代表当前为根目录
            fId = 0;
            folders = fileFolderService.getRootFoldersByFileStoreId(loginUser.getFileStoreId());
            nowFolder = FileFolder.builder().fileFolderId(fId).build();
            location.add(nowFolder);
        } else {
            //当前为具体目录
            folders = fileFolderService.getFileFolderByParentFolderId(fId);
            nowFolder = fileFolderService.getFileFolderById(fId);
            //遍历查询当前目录
            FileFolder temp = nowFolder;
            while (temp.getParentFolderId() != 0) {
                temp = fileFolderService.getFileFolderById(temp.getParentFolderId());
                location.add(temp);
            }
        }
        Collections.reverse(location);
        //获得统计信息
        FileStoreStatistics statistics = myFileService.getCountStatistics(loginUser.getFileStoreId());
        map.put("statistics", statistics);
        map.put("folders", folders);
        map.put("nowFolder", nowFolder);
        map.put("location", location);
        logger.info("网盘页面域中的数据:" + map);
        return "u-admin/upload";
    }

    /**
     * @Description 前往所有文档页面
     * @Author  nanfeng
     * @Date  21:18 2020/3/30
     * @Param [map]
     * @Return java.lang.String
     */
    @GetMapping("/doc-files")
    public String toDocFilePage(Map<String, Object> map) {
        List<MyFile> files = myFileService.getFilesByType(loginUser.getFileStoreId(),1);
        //获得统计信息
        FileStoreStatistics statistics = myFileService.getCountStatistics(loginUser.getFileStoreId());
        map.put("statistics", statistics);
        map.put("files", files);
        map.put("count",files.size());
        return "u-admin/doc-files";
    }

    /**
     * @Description 为前台获取images分页提供数据
     * @Author  nanfeng
     * @Date  21:09 2020/3/30
     * @Param []
     * @Return java.util.Map<java.lang.String,java.lang.Object>
     */
    @ResponseBody
    @GetMapping("/docs")
    public Map<String, Object> toDocs() {
        List<MyFile> files = myFileService.getFilesByType(loginUser.getFileStoreId(),1);
        Map<String,Object> map = new HashMap<String,Object>();
        map.put("code",0);
        map.put("msg","");
        map.put("count",files.size());
        map.put("data", files);
        logger.info("/docs"+"前台请求json数据"+JSON.toJSONString(map));
        return map;
    }

    /**
     * @Description 前往所有图像页面
     * @Author  nanfeng
     * @Date  21:17 2020/3/30
     * @Param [map]
     * @Return java.lang.String
     */
    @GetMapping("/image-files")
    public String toImageFilePage( Map<String, Object> map) {
        List<MyFile> files = myFileService.getFilesByType(loginUser.getFileStoreId(),2);
        //获得统计信息
        FileStoreStatistics statistics = myFileService.getCountStatistics(loginUser.getFileStoreId());
        map.put("statistics", statistics);
        map.put("files", files);
        return "u-admin/image-files";
    }

    /**
     * @Description 为前台获取images分页提供数据
     * @Author  nanfeng
     * @Date  15:22 2020/3/30
     * @Param []
     * @Return java.util.Map<java.lang.String,java.lang.Object>
     */
    @ResponseBody
    @GetMapping("/images")
    public Map<String, Object> toImages() {
        List<MyFile> files = myFileService.getFilesByType(loginUser.getFileStoreId(),2);
        Map<String,Object> map = new HashMap<String,Object>();
        map.put("code",0);
        map.put("msg","");
        map.put("count",files.size());
        map.put("data", files);
        logger.info("/images"+"前台请求json数据"+JSON.toJSONString(map));
        return map;
    }

    /**
     * @Description 前往所有视频页面
     * @Author  nanfeng
     * @Date  21:17 2020/3/30
     * @Param [map]
     * @Return java.lang.String
     */
    @GetMapping("/video-files")
    public String toVideoFilePage( Map<String, Object> map) {
        List<MyFile> files = myFileService.getFilesByType(loginUser.getFileStoreId(),3);
        //获得统计信息
        FileStoreStatistics statistics = myFileService.getCountStatistics(loginUser.getFileStoreId());
        map.put("statistics", statistics);
        map.put("files", files);
        return "u-admin/video-files";
    }

    /**
     * @Description 为前台获取videos分页提供数据
     * @Author  nanfeng
     * @Date  21:15 2020/3/30
     * @Param []
     * @Return java.util.Map<java.lang.String,java.lang.Object>
     */
    @ResponseBody
    @GetMapping("/videos")
    public Map<String, Object> toVideos() {
        List<MyFile> files = myFileService.getFilesByType(loginUser.getFileStoreId(),3);
        Map<String,Object> map = new HashMap<String,Object>();
        map.put("code",0);
        map.put("msg","");
        map.put("count",files.size());
        map.put("data", files);
        logger.info("/videos"+"前台请求json数据"+JSON.toJSONString(map));
        return map;
    }

    /**
     * @Description 前往所有音频页面
     * @Author  nanfeng
     * @Date  21:16 2020/3/30
     * @Param [map]
     * @Return java.lang.String
     */
    @GetMapping("/music-files")
    public String toMusicFilePage( Map<String, Object> map) {
        List<MyFile> files = myFileService.getFilesByType(loginUser.getFileStoreId(),4);
        //获得统计信息
        FileStoreStatistics statistics = myFileService.getCountStatistics(loginUser.getFileStoreId());
        map.put("statistics", statistics);
        map.put("files", files);
        return "u-admin/music-files";
    }

    /**
     * @Description 为前台获取musics分页提供数据
     * @Author  nanfeng
     * @Date  21:16 2020/3/30
     * @Param []
     * @Return java.util.Map<java.lang.String,java.lang.Object>
     */
    @ResponseBody
    @GetMapping("/musics")
    public Map<String, Object> toMusics() {
        List<MyFile> files = myFileService.getFilesByType(loginUser.getFileStoreId(),4);
        Map<String,Object> map = new HashMap<String,Object>();
        map.put("code",0);
        map.put("msg","");
        map.put("count",files.size());
        map.put("data", files);
        logger.info("/musics"+"前台请求json数据"+JSON.toJSONString(map));
        return map;
    }

    /**
     * @Description 前往其他文件页面
     * @Author xw
     * @Date 10:26 2020/2/26
     * @Param [map]
     * @return java.lang.String
     **/
    @GetMapping("/other-files")
    public String toOtherFilePage( Map<String, Object> map) {
        List<MyFile> files = myFileService.getFilesByType(loginUser.getFileStoreId(),5);
        //获得统计信息
        FileStoreStatistics statistics = myFileService.getCountStatistics(loginUser.getFileStoreId());
        map.put("statistics", statistics);
        map.put("files", files);
        return "u-admin/other-files";
    }

    @ResponseBody
    @GetMapping("/others")
    public Map<String, Object> toOthers() {
        List<MyFile> files = myFileService.getFilesByType(loginUser.getFileStoreId(),5);
        Map<String,Object> map = new HashMap<String,Object>();
        map.put("code",0);
        map.put("msg","");
        map.put("count",files.size());
        map.put("data", files);
        logger.info("/others"+"前台请求json数据"+JSON.toJSONString(map));
        return map;
    }

    /**
     * @Description 登录之后的用户主页
     * @Author  nanfeng
     * @Date  21:20 2020/3/30
     * @Param [map]
     * @Return java.lang.String
     */
    @GetMapping("/main")
    public String main(Map<String, Object> map) {
        //获得统计信息
        FileStoreStatistics statistics = myFileService.getCountStatistics(loginUser.getFileStoreId());
        statistics.setFileStore(fileStoreService.getFileStoreById(loginUser.getFileStoreId()));
        map.put("statistics", statistics);
        return "u-admin/index";
    }

    /**
     * @Description 前往帮助页面
     * @Author  nanfeng
     * @Date  21:20 2020/3/30
     * @Param [map]
     * @Return java.lang.String
     */
    @GetMapping("/help")
    public String helpPage(Map<String, Object> map) {
        //获得统计信息
        FileStoreStatistics statistics = myFileService.getCountStatistics(loginUser.getFileStoreId());
        map.put("statistics", statistics);
        return "u-admin/help";
    }
}
